...
pub fn aggregate_bom(
    components: &[crate::circuit_ops::CircuitComponent],
) -> Vec<BomItem> {
    use std::collections::HashMap;

    let mut groups: HashMap<(String, String), Vec<&crate::circuit_ops::CircuitComponent>> = HashMap::new();
    for c in components {
    ...
    items.sort_by(|a, b| a.ref_designators.first().unwrap_or(&String::new()).cmp(b.ref_designators.first().unwrap_or(&String::new())));
    items
}
...
mod tests {
    ...
    fn comp(id: &str, ref_: &str, val: &str) -> CircuitComponent {
        CircuitComponent { id: id.into(), ref_des: ref_.into(), value: val.into(), symbol_id: "resistor".into(), footprint_id: None, x: 0., y: 0., rotation: 0., mirrored: false, mode: "schematic".into() }
    }

    #[test]
    fn aggregates_identical_components() {
        let comps = vec![comp("c1", "R1", "10k"), comp("c2", "R2", "10k"), comp("c3", "R3", "1k")];
