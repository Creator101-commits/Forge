...
// OpenAICompatProvider behaves almost identically to OpenAIProvider,
// but expects a custom base_url and doesn't hardcode api.openai.com
...
mod tests {
    ...
    #[tokio::test]
    async fn test_openai_compat_stream() {
        let mock_server = MockServer::start().await;

        let sse_body = "\
data: {\"id\":\"1\",\"choices\":[{\"delta\":{\"role\":\"assistant\",\"content\":\"\"}}]}\n\
\n\
data: {\"id\":\"1\",\"choices\":[{\"delta\":{\"content\":\"Hello compat\"}}]}\n\
\n\
data: [DONE]\n\
\n\
";
        ...
        let provider = OpenAICompatProvider::new("test-key".to_string(), mock_server.uri());
        ...
        assert_eq!(chunk1.content, "Hello compat");
