...
use crate::pcb_ops::{self, PcbFootprint, PcbPad, PcbTrace, PcbVia, PcbZone, PcbLayer, DrcIssue};
...
pub fn pcb_add_layer(state: State<'_, AppState>, layer: PcbLayer) -> Result<PcbLayer> {
    ...
    conn.execute(
        ...
        rusqlite::params![layer.id, layer.name, layer.kind, layer.color, layer.visible as i32],
    )?;
    Ok(layer)
}
...
pub fn pcb_list_footprints(state: State<'_, AppState>) -> Result<Vec<PcbFootprint>> {
    ...
    let mut s = conn.prepare(
        "SELECT id, component_ref, library_id, x, y, rotation, side FROM pcb_footprint",
    )?;
    let rows = s.query_map([], |r| {
        Ok(PcbFootprint {
            id: r.get(0)?,
...
pub fn pcb_list_pads(state: State<'_, AppState>) -> Result<Vec<PcbPad>> {
    ...
    let conn = open_db(&root)?;
    let mut s = conn.prepare(
        "SELECT id, footprint_id, name, net_id, shape_json, layer_mask FROM pcb_pad",
    )?;
    let rows = s.query_map([], |r| {
        Ok(PcbPad {
            id: r.get(0)?,
...
pub fn pcb_add_trace(state: State<'_, AppState>, trace: PcbTrace) -> Result<PcbTrace> {
    ...
    conn.execute(
        ...
        rusqlite::params![trace.id, trace.net_id, trace.layer_id, trace.points_json, trace.width],
    )?;
    Ok(trace)
}
...
pub fn pcb_list_zones(state: State<'_, AppState>) -> Result<Vec<PcbZone>> {
    let root = active_root(&state)?;
    let conn = open_db(&root)?;
    let mut s = conn.prepare("SELECT id, net_id, layer_id, polygon_json, clearance FROM pcb_zone")?;
    let rows = s.query_map([], |r| {
        Ok(PcbZone {
            id: r.get(0)?,
            net_id: r.get(1)?,
...
pub fn pcb_run_drc(state: State<'_, AppState>) -> Result<Vec<DrcIssue>> {
    ...
    let mut s = conn.prepare("SELECT id, component_ref, library_id, x, y, rotation, side FROM pcb_footprint")?;
    let fps: Vec<PcbFootprint> = s.query_map([], |r| {
        Ok(PcbFootprint {
            id: r.get(0)?, component_ref: r.get(1)?, library_id: r.get(2)?,
            x: r.get(3)?, y: r.get(4)?, rotation: r.get(5)?, side: r.get(6)?,
        })
    })?.collect::<std::result::Result<Vec<_>, _>>()?;

    let mut s = conn.prepare("SELECT id, footprint_id, name, net_id, shape_json, layer_mask FROM pcb_pad")?;
    let pads: Vec<PcbPad> = s.query_map([], |r| {
        Ok(PcbPad {
            id: r.get(0)?, footprint_id: r.get(1)?, name: r.get(2)?,
            net_id: r.get(3)?, shape_json: r.get(4)?, layer_mask: r.get(5)?,
        })
    })?.collect::<std::result::Result<Vec<_>, _>>()?;

    let mut s = conn.prepare("SELECT id, net_id, layer_id, points_json, width FROM pcb_trace")?;
    let traces: Vec<PcbTrace> = s.query_map([], |r| {
        Ok(PcbTrace {
            id: r.get(0)?, net_id: r.get(1)?, layer_id: r.get(2)?,
            points_json: r.get(3)?, width: r.get(4)?,
        })
    })?.collect::<std::result::Result<Vec<_>, _>>()?;

    let mut s = conn.prepare("SELECT id, net_id, x, y, drill, diameter FROM pcb_via")?;
    let vias: Vec<PcbVia> = s.query_map([], |r| {
        Ok(PcbVia {
            id: r.get(0)?, net_id: r.get(1)?, x: r.get(2)?,
            y: r.get(3)?, drill: r.get(4)?, diameter: r.get(5)?,
        })
    })?.collect::<std::result::Result<Vec<_>, _>>()?;
